...
pub fn state() -> Result<CredentialManagerState> {
    ...
    if let Some(profile) = active_profile.as_mut() {
        ...
        codex_configured = active_auth
            .as_deref()
            .is_some_and(|bytes| auth_is_usable_for_profile(bytes, profile))
            && (profile.kind == AuthProfileKind::Personal || managed_config_ready(&config_path));
        if codex_configured {
...
pub fn prepare_profile_activation(profile_id: &str) -> Result<CredentialProfile> {
    ...
    let profile = metadata
        ...
        .cloned()
        .context("授权档案不存在")?;
    if read_profile_auth(&profile)
        .context("授权档案无法读取")?
        .is_none()
        && !(profile.kind == AuthProfileKind::Personal && personal_uses_external_store(&profile))
    {
        anyhow::bail!("授权档案已缺失；百积木档案可重新授权，原授权备份需重新登录后再保存");
    }
    Ok(profile)
}
...
fn checkpoint_active_profile(metadata: &CredentialMetadata) -> Result<()> {
    ...
    } else if !(active.kind == AuthProfileKind::Personal && personal_uses_external_store(active)) {
...
fn personal_uses_external_store(profile: &CredentialProfile) -> bool {
    profile.kind == AuthProfileKind::Personal
        && load_profile_config(profile).ok().is_some_and(|snapshot| {
            !matches!(snapshot.cli_auth_credentials_store.as_deref(), Some("file"))
        })
}
...
fn capture_original_auth_profile(metadata: &mut CredentialMetadata) -> Result<bool> {
    ...
    let external_store = config_path.exists()
        && !matches!(
    ...
    metadata.profiles.push(profile.clone());
    if metadata.active_mode == AuthMode::Chatgpt || metadata.active_profile_id.is_none() {
