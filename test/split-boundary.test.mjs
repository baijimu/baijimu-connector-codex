...
test("desktop manager manifest has no Relay capability surface", async () => {
  ...
  assert.equal(manifest.id, "com.baijimu.connector.codex-desktop");
...
test("desktop manager neither installs CLI nor exposes invoke routes", async () => {
  ...
  assert.match(source, /codex-desktop-artifacts\/v1/);
  assert.doesNotMatch(source, /codex-cli-artifacts/);
});

test("desktop manager imports legacy metadata once and omits redundant cards", async () => {
  const [store, html] = await Promise.all([
    read("src/credential/store.rs"),
    read("ui/index.html"),
  ]);
  assert.match(store, /com\.baijimu\.connector\.codex/);
